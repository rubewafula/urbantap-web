        ...
        if( $postData != null){
            ...
            if(count($user) > 0){
                ...
                DB::update('update user_balance set balance = :balance, transaction_id = :transaction_id where 
                    user_id = :user_id', ['balance'=>$balance, 'transaction_id'=>$transaction->id, 'user_id'=>$user_id]);
            }else{

                DB::insert('insert into user_balance (user_id, balance, transaction_id, 
                    created) values (:user_id, :balance, :transaction_id, now())', ['user_id'=>$user_id, 'balance'=>$balance,
            ...
            DB::insert('insert into payments (reference, date_received, booking_id, 
                    ...
                    (:reference, now(), :booking_id, :paid_by_name,:paid_by_msisdn,:amount,
		     :received_payment,:balance,:status_id, now())', ['reference'=>$transaction_id,
		     'booking_id'=> $invoice_number,"MPESA", 'paid_by_name'=>$name,
		     'paid_by_msisdn'=>$msisdn,'amount'=>$booking_amount,'received_payment'=>$transaction_amount,
                     'balance'=>$balance, 'status_id'=>DBStatus::BOOKING_PAID]);

            DB::insert('insert into booking_trails (:booking_id, :status_id, description, 
                originator, created_at) values (?,?,"MPESA TRANSACTION","MPESA",now())',
                ['booking_id'=>$invoice_number, 'status_id'=>DBStatus::BOOKING_PAID]);
